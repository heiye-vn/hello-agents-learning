...
class MySimpleAgent(SimpleAgent):
    ...
    def _run_with_tools(
        ...
        while current_iteration < max_tool_iterations:
            ...
            if tool_calls:
                ...
                for call in tool_calls:
                    ...
                    # 从响应中移除工具调用标记
                    clean_response = re.sub(r"\[TOOL_CALL:.*?\]", "", response_text)
        ...
        if current_iteration >= max_tool_iterations and not final_response:
            llm_res = self.llm.invoke(messages, **kwargs)
            final_response = (
                llm_res.content if hasattr(llm_res, "content") else str(llm_res)
            )
