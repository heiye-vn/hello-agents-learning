...
from agentscope.formatter import DashScopeMultiAgentFormatter, _dashscope_formatter
from agentscope.message import Msg

# 1. 过滤 Msg 中的 thinking block
_original_get_content_blocks = Msg.get_content_blocks


def _clean_get_content_blocks(self, block_type=None):
    blocks = _original_get_content_blocks(self, block_type)
...
Msg.get_content_blocks = _clean_get_content_blocks

# 2. 修复 AgentScope 格式化中因空 text 产生的 invalid item {"text": None}
_old_reformat = _dashscope_formatter._reformat_messages


def _patched_reformat_messages(messages):
    messages = _old_reformat(messages)
    for msg in messages:
        ...
        if isinstance(content, list):
            new_content = [
                item
                for item in content
                if not (isinstance(item, dict) and item.get("text") is None)
            ]
            if not new_content:
                msg["content"] = ""
            else:
...
_dashscope_formatter._reformat_messages = _patched_reformat_messages


try:
    from prompt_cn import ChinesePrompts
except ModuleNotFoundError:
    from promot_cn import ChinesePrompts
...
class ThreeKingdomsWerewolfGame:
    ...
    async def werewolf_phase(self, round_num: int):
        ...
        async with MsgHub(
            ...
            for i, vote_msg in enumerate(kill_votes):
                # 检查vote_msg是否为None且metadata为dict
                if vote_msg is not None and isinstance(
                    getattr(vote_msg, "metadata", None), dict
                ):
    ...
    async def seer_phase(self):
        ...
        # 检查返回结果是否有效
        if check_result is None or not isinstance(
            getattr(check_result, "metadata", None), dict
        ):
    ...
    async def witch_phase(self, killed_player: str):
        ...
        # 检查返回结果是否有效
        if witch_action is None or not isinstance(
            getattr(witch_action, "metadata", None), dict
        ):
    ...
    async def hunter_phase(self, shot_by_hunter: str):
        ...
        if hunter_agent.name == shot_by_hunter:
            ...
            # 检查返回结果是否有效
            if hunter_action is None or not isinstance(
                getattr(hunter_action, "metadata", None), dict
            ):
...
async def main():
    ...
    api_key = os.getenv("LLM_API_KEY")
    if not api_key:
        print("❌ 未检测到 API Key，请设置环境变量 LLM_API_KEY！")
        print(
            '提示：可在 PowerShell 中运行 $env:LLM_API_KEY="your_key_here" 后重新执行本脚本。'
        )
        return
